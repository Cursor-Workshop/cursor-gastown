...
func (m *SessionManager) Start(polecat string, opts SessionStartOptions) error {
	...
	// Ensure agent settings exist in polecats/ (not polecats/<name>/) so we don't
	// write into the source repo. Agents walk up the tree to find settings.
	polecatsDir := filepath.Join(m.rig.Path, "polecats")
	agentName := opts.Agent
	if agentName == "" {
		// Detect agent from config system
		townRoot := filepath.Dir(m.rig.Path)
		rc := config.ResolveAgentConfig(townRoot, m.rig.Path)
		if rc != nil && rc.Command != "" {
			// Map command to agent name
			switch {
			case strings.Contains(rc.Command, "cursor"):
				agentName = "cursor"
			case strings.Contains(rc.Command, "gemini"):
				agentName = "gemini"
	...
	debugSession("WaitForCommand", m.tmux.WaitForCommand(sessionID, constants.SupportedShells, constants.CursorStartTimeout))

	// Accept bypass permissions warning dialog if it appears
	debugSession("AcceptBypassPermissionsWarning", m.tmux.AcceptBypassPermissionsWarning(sessionID))

	// Wait for Cursor to be fully ready
