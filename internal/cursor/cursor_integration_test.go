...
// TestOrphanConfigCleanup verifies orphan configuration cleanup works correctly.
func TestOrphanConfigCleanup(t *testing.T) {
	t.Run("switch from claude to cursor", func(t *testing.T) {
		tmpDir := t.TempDir()

		// Set up as if Claude was previously used
		...
		// Clean orphan configs for cursor agent
		if err := CleanOrphanAgentConfigs(tmpDir, "cursor"); err != nil {
			t.Fatalf("CleanOrphanAgentConfigs failed: %v", err)
		}

		// .claude should be removed
		if _, err := os.Stat(claudeDir); !os.IsNotExist(err) {
			t.Error(".claude should be removed when switching to cursor")
		}

		// .cursor should still exist
		cursorDir := filepath.Join(tmpDir, ".cursor")
		if _, err := os.Stat(cursorDir); os.IsNotExist(err) {
			t.Error(".cursor should still exist")
		}
	})

	t.Run("preserves user files", func(t *testing.T) {
		tmpDir := t.TempDir()

		// Set up .claude with user-created files
		claudeDir := filepath.Join(tmpDir, ".claude")
		if err := os.MkdirAll(claudeDir, 0755); err != nil {
			t.Fatal(err)
		}
		...
		// Add user file
		if err := os.WriteFile(filepath.Join(claudeDir, "my-notes.md"), []byte("# Notes"), 0644); err != nil {
			t.Fatal(err)
		}
		...
		// Try to clean - should NOT remove because of user file
		if err := CleanOrphanAgentConfigs(tmpDir, "cursor"); err != nil {
			t.Fatalf("CleanOrphanAgentConfigs failed: %v", err)
		}
		...
		if _, err := os.Stat(claudeDir); os.IsNotExist(err) {
			t.Error(".claude should be preserved when it contains user files")
		}
	})
}
