...
func TestBuildResumeCommand(t *testing.T) {
	tests := []struct {
		...
		{
			name:      "cursor with session",
			agentName: "cursor",
			sessionID: "chat-abc123",
			wantEmpty: false,
			contains:  []string{"cursor-agent", "-f", "--resume", "chat-abc123"},
		},
		{
...
func TestCursorAgentPreset(t *testing.T) {
	...
	if info.ProcessNames[0] != "node" {
		t.Errorf("cursor ProcessNames[0] = %q, want node (cursor-agent runs as Node.js)", info.ProcessNames[0])
	}
